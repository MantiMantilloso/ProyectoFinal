...
from src.app_helpers import (
    ...
    crear_mapa, dibujar_zona, dibujar_seb, agregar_leyenda, fmt_km, fmt_m,
)
...
if st.session_state.get("filtros_hash") != filtros_hash:
    for k in ("res2", "buf2_used", "z2_used"):
...
tab1, tab2, tab3, tab4 = st.tabs([
    ...
    "3 - Sensibilidad Geometrica",
    "4 - Frontera de Factibilidad",
])
...
with tab1:
    ...
    st_folium(m1, use_container_width=True, height=500, key="m1")
...
with tab2:
    ...
    col_tog, col_ctrl = st.columns([3, 2])
    with col_tog:
    ...
    with col_ctrl:
        buf2 = st.slider(
            "Expandir zonas (buffer, m)",
            0, 6000, 0, step=100, key="buf2",
            help="Simula un area de exclusion ampliada alrededor de cada parque",
        )
        calc2 = st.button("Calcular SEB restringido", type="primary")

    if calc2:
        with st.spinner("Enumerando aristas activas..."):
            zonas_sel = [
                (n, zonas_dict[n].buffer(buf2) if buf2 > 0 else zonas_dict[n])
                for n in zonas_activas
            ]
            if zonas_sel:
                ...
                res2 = {"estado": "libre", "centro": c_lib2, "radio": r_lib2,
                        "arista_activa": None}
            st.session_state.update(res2=res2, buf2_used=buf2, z2_used=set(zonas_activas))

    res2 = st.session_state.get("res2")
    if res2 is None:
        st.info("Configura las zonas y pulsa 'Calcular SEB restringido' para ver los resultados.")
    else:
        if (st.session_state.get("z2_used") != set(zonas_activas)
                or st.session_state.get("buf2_used") != buf2):
            st.warning("Los parametros cambiaron desde el ultimo calculo. Recalcula para actualizar.")

        estado2   = res2["estado"]
        c_restr2  = res2["centro"]
        r_restr2  = res2["radio"]
        buf2_used = st.session_state.get("buf2_used", 0)
        z2_used   = st.session_state.get("z2_used", set())

        if estado2 == "infactible":
            st.error(
                "F = vacio: las zonas prohibidas expandidas cubren todo Manhattan. "
                "No existe punto factible. Solo se muestra el SEB libre."
            )
        ...
        if res2.get("arista_activa"):
            a = res2["arista_activa"]
            kc[4].metric(
                "Arista vinculante",
                f"{a['fuente']}#{a['arista_idx']}",
        ...
        for nombre, pol in datos["prohibidas"]:
            if nombre in z2_used:
                pol_d = pol.buffer(buf2_used) if buf2_used > 0 else pol
                dibujar_zona(
                    m2, pol_d, color="#dc2626",
        ...
        agregar_leyenda(m2, [
            ("#1e40af", f"SEB libre (r = {fmt_km(r_lib2)})"),
            ("#dc2626",
             f"SEB restringido (r = {fmt_km(r_restr2) if estado2 != 'infactible' else 'N/A'})"),
            ("#dc2626", "Zonas prohibidas activas"),
            ("#9ca3af", "Zonas inactivas"),
        ])
        st_folium(m2, use_container_width=True, height=500, key="m2")

# ══════════════════════════════════════════════════════════════════════════════
# TAB 3 — Sensibilidad Geometrica
# ══════════════════════════════════════════════════════════════════════════════
with tab3:
...
# ══════════════════════════════════════════════════════════════════════════════
# TAB 4 — Frontera de Factibilidad
# ══════════════════════════════════════════════════════════════════════════════
with tab4:
